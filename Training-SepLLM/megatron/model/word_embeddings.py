...
class Embedding(torch.nn.Module):
    ...
    def __init__(
        ...
        self.use_mup = neox_args.use_mup  ##my: Whether to use Microsoft's Mup https://github.com/microsoft/mup. Default for pythia: False
        self.mup_embedding_mult = neox_args.mup_embedding_mult  ##my: Scalar by which we multiply the output of the embedding layer. By default for pythia: 1.0
        self.mup_rp_embedding_mult = neox_args.mup_rp_embedding_mult  ##my: For pythia: mup_rp_embedding_mult=1.0
        ...
        if self.use_pos_emb  or self.USE_BiPE:
            self.embedding_type = neox_args.pos_emb ##my: for pythia: neox_args.pos_emb=rotary
            if (self.embedding_type == "learned") or self.USE_BiPE:
        ...
        if self.num_tokentypes > 0:  ##my: Default for pythia: 0
        ...
        self.opt_pos_emb_offset = neox_args.opt_pos_emb_offset  ##my: For pythia: 0
    ...
    def forward(self, input_ids, position_ids, tokentype_ids=None):  ##my, for USE_BiPE=True, position_ids is intra-position_ids
        # Embeddings.
        words_embeddings = self.word_embeddings(input_ids)

        # if self.use_pos_emb and self.embedding_type in ["learned", "sinusoidal"]: ## Default
        if (self.use_pos_emb and self.embedding_type in ["learned", "sinusoidal"]) or self.USE_BiPE: ##my: For BiPE
            if self.opt_pos_emb_offset:  ##my: pythia: 0
            ...
            position_embeddings.mul_(self.mup_rp_embedding_mult)  ##my: x1.0
        ...
        if self.use_mup:  ##my: pythia default: False
            with torch.no_grad():
    ...
    def forward(self, args):
        if self.USE_SEG_ATTN_ACCELERATOR:
            ...
            if self.USE_BiPE :
                ...
                position_ids = intra_position_ids
                embeddings = super().forward(input_ids, position_ids) ##For BiPE
                return embeddings, inter_position_ids, attention_mask
