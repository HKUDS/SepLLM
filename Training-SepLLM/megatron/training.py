...
def save_base_shapes(neox_args, base_shapes, use_cache):
    ...
    if not neox_args.is_pipe_parallel: ##my  For pythia: neox_args.is_pipe_parallel=True
...
def pretrain(neox_args):
    ...
    # ################################my Debug################################
    # print(f"##################Debug lr_scheduler.num_iters: {lr_scheduler.num_iters}##########################")
    # ##################################################################
...
def forward_step(
    ...
    """Forward step."""
    if neox_args.is_pipe_parallel:  ##my  For pythia: neox_args.is_pipe_parallel=True
    ...
    # maybe_tuple = model(forward_input, neox_args=neox_args)
